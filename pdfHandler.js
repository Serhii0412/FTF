...
async function handlePdfOrder(driverId, pdfFilePath, bot) {
    ...
    await bot.sendDocument(driverId, filledPdfPath, { caption: 'Ваш заказ с заполненными данными' });
}
...
function sendPhotosToDriver(driverId, bot) {
    ...
    if (photos.length < 8) {
        bot.sendMessage(driverId, 'Для отправки заказов необходимо минимум 8 фотографий.');
        return;
    }
...
function getPhotos() {
    ...
    return fs.readdirSync(photosDirectory).map(fileName => path.join(photosDirectory, fileName));
}
